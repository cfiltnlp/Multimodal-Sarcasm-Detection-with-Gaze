'''In this model we try to separate the classification heads for sarcasm-type and emotion-class
while they are both trained together commonly with same layers'''
...
"""This file contains the model we have used for -
1. Multiclass Implicit Emotion Classification and Sarcasm Type information
...
for
    ...
    d. num_classes=4 (Sarcasm type detection)

We have 2 different output layers one for implicit emotion and the other for sarcasm type
"""
...
class Speaker_Independent_Triple_Mode_with_Context(nn.Module):
    def __init__(self, input_embedding_A=2048, input_embedding_B=1024, input_embedding_C=audio_embedding_size, shared_embedding=1024, projection_embedding=512, dropout=0.5, num_classes=5):
        ...
        self.emo_head = nn.Linear(128,  self.num_classes)
        self.sarctype_head = nn.Linear(128,4)
    ...
    def forward(self, uA, cA, uB, cB, uC, cC):
        ...
        common_output = self.shared_layers(input)
        return [self.emo_head(common_output),self.sarctype_head(common_output)]
