...
import emotion_classification_model
...
mustard_input = pd.read_csv(
    '/home/development/apoorvan/AN_MTP/SER/data/sarcasm_detection_mustard_pp.csv', index_col=0)


# print(mustard_input.columns)

temp = open(path+'extracted_features/final_all_features.pickle', 'rb')
...
#                     f'Epooch:{best_epooch} | Train Loss: {loss.detach().item():.3f} | Valid loss: { valid_loss.detach().item():7.3f} | Valid F1: { valid_f1:7.3f}')
            ...
            if abs(train_losses[-2]-train_losses[-1]) < 0.0001:
                ...
                patience_flag = 1
    return evaluation(valid_loader, best_model, call, report, True), best_model
...
print('splits', len(split))
# print(mustard_input.columns)


# command = '-s n -m VTA -c y'
...
# args = argParser.parse_args(command.split())
...
# f = open('/home/development/apoorvan/AN_MTP/cikm/log/sarcasm/' +
#          filename+'.txt', 'w')
...
for dropout in [float(args.dropout)]:
    for lr in [float(args.learning_rate)]:
        for batch_size in [int(args.batch_size)]:
            for shared_embedding_size, projection_embedding_size in zip([1024], [512]):
                ...
                #     print(mod)
                    ...
                    (true, pred), best_model = training(mod=mod, criterion=criterion, optimizer=optimizer, call=COMMAND,
                                                 train_loader=train_loader, valid_loader=valid_loader, fold=fold, e=epooch, patience=patience)
                    best_model.to('cpu')
                    torch.save(best_model.state_dict(), '/home/development/apoorvan/AN_MTP/cikm/saved_models/sarcasm/' +
...
#                 stats.to_csv('/home/development/apoorvan/AN_MTP/cikm/stats/sarcasm/' +
...
#                 results.to_csv('/home/development/apoorvan/AN_MTP/cikm/predictions/sarcasm/' + args.mode+'/' +
